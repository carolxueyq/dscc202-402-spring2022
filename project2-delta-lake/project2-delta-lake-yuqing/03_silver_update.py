...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT count(heartrate) as number_of_negative_heart_rate, p_eventdate as event_date
# MAGIC FROM broken_readings
# MAGIC GROUP BY p_eventdate

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC Select sum(counts) 
# MAGIC   from (select count(heartrate) as counts
# MAGIC           from broken_readings
# MAGIC       group by p_eventdate); 
...
# TODO
assert updatesDF.schema == broken_readings.schema, "Schemas do not match"
...
# TODO
late = spark.read.json(rawPath + "/late")
late.count()
